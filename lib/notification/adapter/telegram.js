...
import { buildFredyListingUrl } from '../fredyUrl.js';
...
function buildHtmlBody(jobName, serviceName, o, baseUrl) {
  ...
  const fredyUrl = buildFredyListingUrl(baseUrl, o.id);
  const fredyLink = fredyUrl ? `\n<a href='${escapeHtml(fredyUrl)}'>Open in Fredy</a>` : '';
  return (
...
function buildPlainCaption(jobName, serviceName, o, baseUrl) {
  ...
  const meta = [o.address, o.price, o.size, o.commute].filter(Boolean).join(' | ');
  const fredyUrl = buildFredyListingUrl(baseUrl, o.id);
  const fredyLine = fredyUrl ? `\nOpen in Fredy: ${fredyUrl}` : '';
