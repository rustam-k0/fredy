...
import { nextClockBoundary } from './schedule.js';
...
export function initJobExecutionService({ providers, intervalMs }) {
  ...
  // takes effect without requiring a restart. Align every run to a wall-clock boundary instead of
  // waiting one full interval after the previous run finishes. With the configured 60-minute
  // interval this means 10:00, 11:00, 12:00, ... even when a scan itself takes several minutes.
  if (Number.isFinite(intervalMs) && intervalMs > 0) {
    const scheduleNext = async () => {
      const liveSettings = await getSettings();
      const minutes = Number(liveSettings?.interval);
      const effectiveMinutes = Number.isFinite(minutes) && minutes > 0 ? minutes : intervalMs / 60000;
      const nextDelay = nextClockBoundary(effectiveMinutes) - Date.now();
    ...
    scheduleNext().catch((err) => {
      logger.error('Failed to schedule job execution', err);
    });
  }
  // Do not run automatically on startup. Restarts can happen at any minute and an immediate scan
  // would break the wall-clock schedule. Administrators can still use "Search now" explicitly.
