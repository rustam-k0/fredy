...
export const getKnownListingHashesForJobAndProvider = (jobId, providerId) => {
  return SqliteConnection.query(
    `SELECT DISTINCT l.hash
     FROM listings l
            JOIN jobs source_job ON source_job.id = l.job_id
            JOIN jobs target_job ON target_job.id = @jobId
     WHERE source_job.user_id = target_job.user_id
       AND l.provider = @providerId`,
    { jobId, providerId },
  ).map((r) => r.hash);
};
